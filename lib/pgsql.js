...
import CoreCron from "#core/cron";
...
` );
    ...
    #backup () {
        if ( !this.isPrimary || !this.#config.backup?.cron || !this.#config.backup?.numnerOfBackups || !this.#config.replication?.username ) return;

        fs.mkdirSync( this.#backupDir, { "recursive": true } );
        fs.chownSync( this.#backupDir, this.#uid, this.#gid );
        fs.chmodSync( this.#backupDir, 0o700 );

        this.#backupCron = new CoreCron( this.#config.backup.cron ).on( "tick", this.#makeBackup().bind( this ) ).unref().start();
    }
    ...
    #makeBackup () {
        ...
        if ( backups.length <= this.#config.backup?.numnerOfBackups ) return;

        for ( const backup of backups.slice( this.#config.backup?.numnerOfBackups ) ) {
