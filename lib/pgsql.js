    ...
    #writeReplicationsConfig ( clear ) {
        ...
        if ( !clear ) {
            if ( this.isPrimary ) {
                ...
                replicationConfig += `primary_conninfo = 'host=${this.replicationHostname} port=${this.config.replication.port} user=${this.config.replication.username} password=${this.config.replication.password} application_name=${this.syncStandbyName}'\n`;
            }
            else {
