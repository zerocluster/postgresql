...
export default class Pgsql extends Events {
    ...
    get syncStanbdbuyNames () {
        ...
        if ( this.maxNumberOfSyncStanbdbuys ) {
            ...
            for ( let n = 1; n <= this.maxNumberOfSyncStanbdbuys; n++ ) {
    ...
    async run () {
        ...
        console.log( `Level: ${this.#level}, id: ${this.id || "-"}, name: ${this.name || "-"}, sync standbuy names: ${this.syncStanbdbuyNames || "-"}` );
    ...
    async #initPrimary () {
        ...
        if ( !fs.existsSync( this.#dataDir + "/PG_VERSION" ) ) {
            ...
            childProcess.execFileSync( "initdb", ["--encoding", "UTF8", "--no-locale", "-U", "postgres", "--pwfile", pwfile, "-D", this.#dataDir], { "uid": this.#uid, "gid": this.#gid, "encoding": "utf8" } );
...
` );
    ...
    #writeReplicationsConfig ( clear ) {
        ...
        if ( !clear ) {
            if ( this.maxNumberOfSyncStanbdbuys ) {
                replicationConfig += `synchronous_standby_names = 'FIRST ${this.maxNumberOfSyncStanbdbuys} ( ${this.syncStanbdbuyNames} )'`;
            }
