...
export default class Pgsql extends Events {
    ...
    get syncStandbyName () {
        this.#syncStandbyName ??= this.isSyncStandby ? uuidV4().replaceAll( "-", "_" ) : "";

        return this.#syncStandbyName;
    }
    ...
    async run () {
        ...
        console.log( `Level: ${this.#level}, id: ${this.id || "-"}, name: ${this.name || "-"}` );
    ...
    #init () {
        if ( this.level === 1 ) {
            if ( this.id && this.id > this.config.replication.level1.maxNumberOfSyncStanbdbuys ) {
                return result( [400, `Standby id for level 1 is not valid`] );
            }
        }
        else if ( this.level === 2 ) {
            if ( this.id && this.id > this.config.replication.level2.maxNumberOfSyncStanbdbuys ) {
                return result( [400, `Standby id for level 2 is not valid`] );
            }
        }

        this.#dataRoot = this.config.dataRoot;
        ...
        this.#backupDir = path.join( this.#dataRoot, "backups", this.config.postgresVersion + "", this.config.postgresClusterName );
    ...
    #writeReplicationsConfig ( clear ) {
        ...
        if ( !clear ) {
            if ( this.level ) {
                ...
                let connInfo = `primary_conninfo = 'host=${hostname} port=${this.#config.replication.port} user=${this.#config.replication.username} password=${this.#config.replication.password}`;

                if ( this.name ) {
                    connInfo += ` application_name=${this.name}`;
                }

                replicationConfig += connInfo + "\n";
            }

            if ( this.maxNumberOfSyncStanbdbuys ) {
                replicationConfig += `synchronous_standby_names = 'FIRST ${this.maxNumberOfSyncStanbdbuys} ( * )'\n`;
            }
        }
        ...
        if ( fs.existsSync( autoConfigPath ) ) {
            ...
            config = config
                .split( "\n" )
                .map( line => line.trim() )
                .filter( line => !line.startsWith( "primary_conninfo" ) )
                .join( "\n" );
