...
` );
    ...
    #writePostgresConfig () {
        ...
        let maxNumberOfSyncStanbdbuys;

        if ( !this.leval ) {
            maxNumberOfSyncStanbdbuys = this.config.replication?.level1?.maxNumberOfSyncStanbdbuys;
        }
        else if ( this.leval === 1 ) {
            maxNumberOfSyncStanbdbuys = this.config.replication?.level2?.maxNumberOfSyncStanbdbuys;
        }

        if ( maxNumberOfSyncStanbdbuys ) {
            replicationConfig.synchronous_standby_names = `FIRST ${maxNumberOfSyncStanbdbuys} ( s1, s2 )`;
        }
        ...
        // merge configs
        const config = mergeObjects( {}, this.config.settings, replicationConfig );
