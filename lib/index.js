...
export default class {
    ...
    async run () {
        ...
        for ( const access of config.config.access ) {
            pgHba.push( `${access.host ?? ""} ${access.database ?? ""} ${access.user ?? ""} ${access.address ?? ""} ${access.auth ?? ""} ${access.auth_options ?? ""}` );
        }
        ...
        const settings = [];
        for ( const name of config.config.settings ) settings.push( `${name} = ${config.config.settings[name] == null ? "" : "'" + config.config.settings[name] + "'"}` );
        fs.writeFileSync( dataDir + "/conf.d/100-init.conf", settings.join( "\n" ) );
