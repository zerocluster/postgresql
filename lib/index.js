...
export default class {
    ...
    async run () {
        ...
        if ( config?.config?.settings ) {
            if ( typeof config.config.settings === "string" ) {
                settings.push( config.config.settings );
            }
            else {
                for ( const name in config.config.settings ) {
                    const value = config.config.settings[name];
        ...
        // link unix sockets directory
        if ( !fs.existsSync( dataRoot + "/sockets" ) ) {
            console.log( "--------", unixSocketDirectories, dataRoot + "/sockets" );
            fs.symlinkSync( unixSocketDirectories, dataRoot + "/sockets" );
        }
