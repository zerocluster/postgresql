...
CREATE SEQUENCE IF NOT EXISTS cron.schedule_id_seq AS int4 CYCLE;
...
CREATE TRIGGER schedule_after_update AFTER UPDATE ON cron.schedule FOR EACH ROW EXECUTE FUNCTION schedule_after_update_trigger();
...
CREATE POLICY fp_u ON cron.schedule FOR ALL USING ( username = CURRENT_USER );
...
export default class {
    ...
    async #runTask ( task, cron ) {
        if ( task.started ) {
            ...
            if ( task.as_superuser ) dbh = this.#dbh._newConnection( { "database": task.username } );
            else dbh = this.#dbh._newConnection( { "username": task.username } );
